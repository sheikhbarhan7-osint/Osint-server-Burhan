...
pending_download: Dict[str, dict] = {}   # /download flow: token -> {url,title,artist,duration,platform,qualities,chat_id}
_np_tasks       : Dict[int, asyncio.Task] = {}   # chat_id → live updater task
...
def _yt_audio_dl(query_or_url: str, is_url: bool = False) -> Optional[dict]:
    ...
    uid  = uuid.uuid4().hex
    tmpl = os.path.join(TMP_DIR, f"{uid}.%(ext)s")
    for player in [['android_creator'], ['android_testsuite'], ['ios'], ['android'], []]:
        opts = {
...
def _yt_video_dl(url: str, quality: str) -> Optional[dict]:
    ...
    if os.path.exists(COOKIES_FILE): opts['cookiefile'] = COOKIES_FILE
    for player in [['android_creator'], ['android_testsuite'], ['ios'], []]:
        if player: opts['extractor_args'] = {'youtube': {'player_client': player}}
        try:
            with yt_dlp.YoutubeDL(opts) as ydl:
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  /download HELPERS  (any yt-dlp-supported link: YouTube, Instagram, ...)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def _yt_search_resolve(query: str) -> Optional[str]:
    """Resolve a plain-text query to a YouTube watch URL (used by /download when no link is given)."""
    ...
    try:
        ...
        logger.debug(f"yt_search_resolve: {e}")
    return None


def _generic_info(url: str) -> Optional[dict]:
    """Fetch metadata + all available formats for any yt-dlp supported URL (YouTube, Instagram, etc.)."""
    base_opts = {
    ...
    clients = [['android_creator'], ['android_testsuite'], ['ios'], ['android'], []] if is_yt else [[]]
    for player in clients:
        ...
        try:
            ...
            logger.debug(f"generic_info {player}: {e}")
    return None


def _available_qualities(info: dict) -> List[str]:
    """Return the distinct video heights actually present in the source, e.g. ['1080p','720p','480p']."""
    heights = set()
...
def _generic_audio_dl(url: str) -> Optional[dict]:
    """Download best audio (as mp3) from any yt-dlp supported URL — YouTube, Instagram, etc."""
    uid     = uuid.uuid4().hex
    tmpl    = os.path.join(TMP_DIR, f"{uid}.%(ext)s")
    is_yt   = _is_youtube_url(url)
    clients = [['android_creator'], ['android_testsuite'], ['ios'], ['android'], []] if is_yt else [[]]
    for player in clients:
        ...
        try:
            ...
            logger.debug(f"generic_audio {player}: {e}")
        for f in os.listdir(TMP_DIR):
            if f.startswith(uid):
                try: os.remove(os.path.join(TMP_DIR, f))
                except: pass
...
def _generic_video_dl(url: str, quality: str) -> Optional[dict]:
    """Download video at a specific quality — 'quality' must be one that actually exists on the source
    (or 'best'), as returned by _available_qualities()."""
    uid  = uuid.uuid4().hex
    tmpl = os.path.join(TMP_DIR, f"{uid}.%(ext)s")
    if quality == 'best':
        fmt = 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best'
        ...
        fmt = (f'bestvideo[height<={h}][ext=mp4]+bestaudio[ext=m4a]'
               f'/best[height<={h}][ext=mp4]/best[height<={h}]/best')
    is_yt   = _is_youtube_url(url)
    clients = [['android_creator'], ['android_testsuite'], ['ios'], []] if is_yt else [[]]
    for player in clients:
        opts = {
            'quiet': True, 'no_warnings': True, 'nocheckcertificate': True, 'geo_bypass': True,
            'format': fmt, 'outtmpl': tmpl,
        ...
        try:
            ...
            logger.debug(f"generic_video {player} {quality}: {e}")
        for f in os.listdir(TMP_DIR):
            if f.startswith(uid):
                try: os.remove(os.path.join(TMP_DIR, f))
                except: pass
...
# ── Try to import AudioQuality (pytgcalls ≥ 1.x) ─────────────────────────────
try:
...
def _make_stream(fp: str, seek_sec: int = 0):
    """Build a MediaStream, using AudioQuality.STUDIO when available."""
...
async def _np_live_updater(chat_id: int, msg_id: int, bot):
    """Background task: edits the Now Playing message every 5 s with live progress."""
    UPDATE_INTERVAL = 5
    await asyncio.sleep(UPDATE_INTERVAL)   # first update after 5 s
    while True:
        ch = active_chats.get(chat_id, {})
        # stop if song changed, stopped, or paused
        ...
        if not ch.get('paused'):
            try:
                ...
                pass   # message deleted / unchanged / flood → skip silently
        await asyncio.sleep(UPDATE_INTERVAL)


def _start_np_updater(chat_id: int, msg_id: int, bot):
    """Cancel any existing updater for this chat and start a fresh one."""
...
async def vc_play_item(chat_id: int, item: dict, bot, seek_sec: int = 0):
    """
    Play a queue item.  Returns (True, '') on success or (False, error_str) on failure.
    IMPORTANT: only leave_group_call when we are already streaming — calling it
    before the first join corrupts pytgcalls internal state and the stream never starts.
    """
    fp       = item['file']
    ...
    # Cancel live updater for the old song
    _stop_np_updater(chat_id)

    # ── If already streaming, cleanly stop it first ──────────────────────────
    if ch.get('playing'):
    ...
    try:
        ...
        # Apply saved volume (non-fatal if unsupported)
...
async def cmd_play(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── YouTube URL → just fetch & play its audio, no quality prompt ─────────
    ...
    if private:
        ...
        # ── If something is already playing → STOP IT, clear queue, play new ──
        ch = active_chats.get(chat_id, {})
        if ch.get('playing'):
            await msg.edit_text(
                f"{BRAND}\n{DIVIDER}\n\n"
...
async def cmd_download(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/download <song name | YouTube link | Instagram link>
    Lets the user pick MP3 or Video. If Video is picked, shows only the
    qualities that actually exist for that upload (fetched live via yt-dlp)."""
    auth, reason = check_auth(update)
    if not auth:
        await update.message.reply_text(unauthorized_msg(update, reason), parse_mode='Markdown')
        return
    ...
    # ── Resolve to a URL (plain text → search YouTube for one) ───────────────
    ...
    if not url:
        ...
        if not url:
            await msg.edit_text(
                ...
                f"Try a different name, or paste a YouTube/Instagram link.",
                parse_mode='Markdown'
            )
            return

    # ── Fetch metadata + real available formats ───────────────────────────────
    try:
        info = await asyncio.wait_for(_run_in_pool(_generic_info, url), timeout=30)
    except asyncio.TimeoutError:
    ...
    plat_icon = {'youtube': '🔴', 'instagram': '📸'}.get(platform, '🔗')
...
async def cmd_skip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ch.get('playing'):
        ...
        return
    # stop current stream, then play next
    for fn in ('stop_stream', 'leave_group_call'):
        try:
            await getattr(calls, fn)(chat_id)
            break
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"{BRAND}\n{DIVIDER}\n\n"
        f"📋 *All Commands:*\n\n"
        f"🎵 `/play <name or YouTube URL>` — Just plays the song (VC / DM audio)\n"
        f"⬇️ `/download <name or YouTube/Instagram URL>` — Get MP3 or Video (pick quality)\n"
...
async def _send_download_result(update, context, q, result: dict, pv: dict, is_video: bool):
    """Send the downloaded file (mp3 or mp4) produced by the /download flow, then clean up."""
    fp       = result['file']
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── /download: format type chosen  (dt|token|mp3|video) ───────────────────
    ...
    # ── /download: quality chosen for video  (dq|token|quality) ───────────────
